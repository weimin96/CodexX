...
- 新增周期预热，合并原 5 小时预热和 7 天预热入口。
- 新增用户文档、发布可信度说明、CI workflow、Dependabot 配置和 Release `SHA256SUMS`。

### 修改

- 周期预热不再校验剩余额度是否为 100%，只判断周期窗口是否尚未进入倒计时。
- README 补齐安装平台、数据位置、安全说明和常见问题。

### 安全

- 收紧 Tauri CSP 和默认 capability。
- 加强启动数据库初始化、窗口关闭行为缓存和 OAuth 本地回调校验。

## 0.1.6 - 2026-04-26

### 修复

- 修复 Token 用量统计在相似账号之间归属错误的问题。
